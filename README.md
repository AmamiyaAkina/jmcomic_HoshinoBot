...
本插件原项目地址：[jmcomic_HoshinoBot](https://github.com/SlightDust/jmcomic_HoshinoBot)<br>
HoshinoBot [原项目地址](https://github.com/Ice9Coffee/HoshinoBot)<br>
HoshinoBot [作品插件索引](https://hoshino-plugins.pcrbot.com/)
...
4. 前往`jmcomic_HoshinoBot.py`配置你的HTTP HTTPS代理（如果不需要配置走梯下本的话，可跳过）。
...
`jm_config.yml`的`base_dir`是下载的全部图片文件的路径，`pdf_dir`是输出pdf的路径，如果是Windows用的话要改成Windows的路径格式(反斜杠太不优雅了)。加密PDF、压缩包均会保存至`pdf_dir`。<br>
...
默认情况下，文件密码是jmid的倒序，可以改`enctypt_pdf`和`encrypt_zip`的`password`传参设置别的密码。  
...
2025.05.09 可选上传：1.未加密pdf；2.加密pdf；3.加密zip+未加密pdf；4.加密zip+加密pdf。增加依赖`pyzipper`。  
2025.05.10 增加了一个记录正在下载的列表，尝试下载已经在下载中的本子时发提示并return。优化提示。  

2026.02.09 增加了代理配置，增加了下载时读取漫画名。<br>
2026.03.12 回滚代码方案，手动更新了一波API domain，但注释保留了终端全局代理方案。


## 杂谈
原本为了应对上游API炸了，所以写了代理支持，用了两三个月吧，结果自动登录插件会403了，cookies的自动抓取，也抓不到登录状态的cookies，改了半天有点无从下手，所以在功能层面不得不回滚了API方案，凑巧也看见有新的API地址了，然后就，回滚解决了？<br>我承认我此前说话有点大声了点，在这里先给项目原作者和API维护者磕头了，但如果能实现稳定登录，那通过本地配置代理的方式，在长远与稳定的角度来看，或许会是更好的选择。但还是那句话，且用且珍惜吧。
